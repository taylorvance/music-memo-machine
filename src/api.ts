...
export function saveClip(
  ...
  return request<{ session: Session; clip: Clip }>(
...
export function updateClip(
  id: string,
  patch: Partial<Pick<Clip, 'title' | 'notes' | 'sync_state'>>,
) {
...
export function restoreClip(id: string) {
  return request<{
    clip: Clip;
    session: Session | null;
    restored_clip_id: string;
    source_state: string;
  }>(`/api/trash/clips/${id}/restore`, {
    method: 'POST',
  });
}

export function restoreSession(id: string) {
  return request<{ session: Session; restored_session_id: string }>(
...
export function compressCandidates() {
  return request<{ compressed_session_ids: string[]; storage: StorageSummary }>(
    '/api/storage/compress-candidates',
    {
      method: 'POST',
    },
  );
...
export function degradeThrowaways() {
  return request<{ degraded_session_ids: string[]; storage: StorageSummary }>(
    '/api/storage/lossy-throwaways',
    {
      method: 'POST',
    },
  );
