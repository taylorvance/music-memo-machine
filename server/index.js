...
async function loadSessions() {
  ...
  const sessions = await Promise.all(
    store.loadSessionsMetadata().map((session) => enrichSession(session)),
  );
...
async function trashClipFileArtifacts(clip) {
  ...
  await moveIfExists(
    ...
    path.join(trashDir, clip.audio_path),
  );
  await moveIfExists(
    path.join(libraryRoot, 'clips', `${clip.id}.json`),
    path.join(trashDir, `${clip.id}.json`),
  );
...
async function trashSessionFileArtifacts(session) {
  ...
  await fs.mkdir(trashDir, { recursive: true });
  await moveIfExists(
    path.join(libraryRoot, 'sessions', session.id),
    path.join(trashDir, 'session'),
  );
  await moveIfExists(
    path.join(libraryRoot, 'cache', 'waveforms', `${session.id}.json`),
    path.join(trashDir, 'waveform.json'),
  );
...
async function restoreSessionFileArtifacts(sessionId) {
  ...
  await moveIfExists(
    path.join(trashDir, 'session'),
    path.join(libraryRoot, 'sessions', sessionId),
  );
  await moveIfExists(
    path.join(trashDir, 'waveform.json'),
...
async function loadTrashedSessions() {
  ...
  for (const entry of entries) {
    ...
    const manifest = await loadSessionTrashManifest(entry.name).catch(
      () => null,
    );
...
async function purgeExpiredTrashKind(kind, loadManifest, trashDirForId, now) {
  ...
  for (const entry of entries) {
    ...
    if (
      ...
      Date.parse(manifest.purge_after) > now.getTime()
    )
      continue;
...
async function purgeExpiredTrash(now = new Date()) {
  const [sessions, clips] = await Promise.all([
    purgeExpiredTrashKind(
      'sessions',
      loadSessionTrashManifest,
      sessionTrashDir,
      now,
    ),
...
function sessionStateAfterClipDelete(session) {
  ...
  if (
    (session.bookmarks || []).some(
      (bookmark) => bookmark.state === 'unresolved',
    )
  ) {
    return {
      state: 'bookmarked',
...
function classifySession(session, sim) {
  ...
  const compressionCandidate =
    ...
    ((isThrowaway && age >= policy.unbookmarked_compression_days) ||
      (session.retention_class === 'archival_context' &&
        age >= policy.archival_compression_days));
  const lossyCandidate =
    policy.lossy_only_throwaway &&
    isThrowaway &&
    !hasBookmarks &&
    !hasClips &&
    age >= policy.unbookmarked_retention_days;
  ...
  return {
    ...
    estimated_compression_savings_bytes: compressionCandidate
...
function summarizeStorage(sessions, clips, sim) {
  ...
  const clipBytes = clips.reduce(
    (sum, clip) => sum + (clip.storage_size_bytes || 0),
    0,
  );
  const cacheBytes = Math.max(
    2 * 1024 * 1024,
    Math.floor((sourceBytes + archivalBytes) * 0.006),
  );
  const safeDeleteBytes = classifications
    .filter((item) => item.safe_delete)
    .reduce((sum, item) => sum + item.size_bytes, 0);
  const compressionBytes = classifications.reduce(
    (sum, item) => sum + item.estimated_compression_savings_bytes,
    0,
  );
  const unsyncedClips = clips.filter(
    (clip) => clip.sync_state !== 'synced',
  ).length;
...
async function trimWavFile(sourcePath, targetPath, startSeconds, endSeconds) {
  ...
  const startByte =
    data.offset + Math.floor((safeStart * byteRate) / blockAlign) * blockAlign;
  const endByte =
    data.offset + Math.floor((safeEnd * byteRate) / blockAlign) * blockAlign;
...
app.post('/api/sessions/:id/clips', async (req, res, next) => {
  try {
    ...
    const requestedResolvedIds = new Set([
      ...(Array.isArray(req.body.resolved_bookmark_ids)
        ? req.body.resolved_bookmark_ids
        : []),
    ...
    const clip = {
      ...
      storage_size_bytes: Math.max(
        stat.size,
        Math.round((end - start) * 10 * 1024 * 1024),
      ),
    };

    const unresolvedBookmarks = (session.bookmarks || []).filter(
      (bookmark) => bookmark.state === 'unresolved',
    );
    const resolvedBookmarkIds = new Set(
      unresolvedBookmarks
        .filter(
          (bookmark) =>
            bookmark.timestamp_seconds >= start &&
            bookmark.timestamp_seconds <= end,
        )
    ...
    if (resolvedBookmarkIds.size === 0) {
      const trailingBookmark = unresolvedBookmarks
        ...
        .filter(
          (bookmark) =>
            bookmark.timestamp_seconds <= end + postTakeBookmarkGraceSeconds,
        )
    ...
    session.bookmarks = (session.bookmarks || []).map((bookmark) => {
      if (
        bookmark.state === 'unresolved' &&
        resolvedBookmarkIds.has(bookmark.id)
      ) {
        return {
          ...bookmark,
          state: 'resolved',
    ...
    session.retention_class = 'archival_context';
    session.sync_state =
      session.sync_state === 'synced' ? 'pending_sync' : session.sync_state;
...
app.delete('/api/clips/:id', async (req, res, next) => {
  try {
    ...
    if (session) {
      session.clips = (session.clips || []).filter(
...
app.post('/api/trash/sessions/:id/restore', async (req, res, next) => {
  try {
    ...
    await fs.rm(sessionTrashDir(req.params.id), {
      recursive: true,
      force: true,
    });
    ...
    res.json({
      ...
      restored_session_id: req.params.id,
    });
  } catch (error) {
    next(error);
  }
...
app.get('/api/storage', async (_req, res, next) => {
  try {
    await purgeExpiredTrash();
    const [sessions, clips, sim] = await Promise.all([
      loadSessions(),
      loadClips(),
      loadStorageSim(),
    ]);
...
app.patch('/api/storage', async (req, res, next) => {
  try {
    ...
    if (Number.isFinite(Number(req.body.free_bytes))) {
      sim.simulated_free_bytes = Math.max(
        0,
        Math.min(sim.total_bytes, Number(req.body.free_bytes)),
      );
    }
...
app.post('/api/storage/delete-safe', async (_req, res, next) => {
  try {
    ...
    const safeIds = summary.candidates
      .filter((candidate) => candidate.safe_delete)
...
app.post('/api/storage/compress-candidates', async (_req, res, next) => {
  try {
    ...
    const candidateIds = new Set(
      summary.candidates
        .filter((candidate) => candidate.compression_candidate)
        .map((candidate) => candidate.id),
    );
...
app.post('/api/storage/lossy-throwaways', async (_req, res, next) => {
  try {
    ...
    const candidateIds = new Set(
      summary.candidates
        .filter((candidate) => candidate.lossy_candidate)
        .map((candidate) => candidate.id),
    );
